...
namespace base {
    ...
    auto ConfigManager::unpackSingle(PathCref path, SizeT lineNum, StrViewCref line) const -> StrView {
    ...
    auto ConfigManager::unpackMulti(
            PathCref path,
            SizeT lineNum,
            StrViewCref line,
            Char8 delim,
            SizeT required) const -> StrvVector
    {
        auto vec     = StrvVector();
        auto strPath = path.string();
        auto ptr     = line.cbegin();
    ...
    void ConfigManager::deleteComments(PathCref path, StrvVector& lines) const {
        auto strPath = path.string();
    ...
    auto ConfigManager::pairFromLine(PathCref path, SizeT lineNum, StrViewCref line, Char8 delim) const -> StrViewPair {
    ...
    void ConfigManager::preprocessorTask(PathCref path, SizeT lineNum, StrViewCref line) {
        auto strPath = path.string();
        auto ptr = line.cbegin();
    ...
    auto ConfigManager::unpackVariables(PathCref path, SizeT lineNum, StrViewCref str) -> ftl::String {
        auto res = ftl::String();
            ...
            if (!onSingleQuotes && !onDoubleQuotes) {
                if (*ptr == '$' && firstSymbolFlag) {
                    ...
                    if (onReadVar && start != ptr) {
                        onReadVar = false;
                        res += cfg_detls::cfgData().getValue(
                                cfg_detls::GLOBAL_NAMESPACE, str.substr(start - str.cbegin(), ptr - start));
                    }
                    if (*ptr == ',')
                        res += ',';
                    continue;
    ...
    void ConfigManager::parseLinesTask(PathCref path, StrvVector& lines) {
        ...
        for (SizeT n = 0; n < lines.size(); ++n) {
            ...
            if (*ptr == '[') {
                ...
                if (currentSection) {
                    ...
                    cfgData().addValue(cfg_detls::GLOBAL_NAMESPACE, pair.first, realVars);
                }
            }
        }
    }
    ...
    void ConfigManager::processFileTask(PathCref path) {
        auto file  = readFileToString(path, true); // no log mode
