...
namespace base {
    ...
    DE_MARK_AS_SINGLETON(CpuExtensionsChecker);
    };


    inline CpuExtensionsChecker::~CpuExtensionsChecker() = default;

    inline CpuExtensionsChecker::CpuExtensionsChecker() {
        int info[4];
        cpuid(info, 0);
        int nIds = info[0];
    ...
    const auto& cpu_extensions_checker() {
        return CpuExtensionsChecker::instance();
    }
}